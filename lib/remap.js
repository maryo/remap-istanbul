...
define([
	...
	return function remap(coverage, options) {
		...
		var instrument = {sourceMaps: {}};

		try {
			instrument = require('./webpackInstrumenterLoader');
		} catch (e) {}
		...
		coverage.forEach(function (item) {
			Object.keys(item).forEach(function (filePath) {
				...
				if (instrument.sourceMaps[filePath]) {
					rawSourceMap = instrument.sourceMaps[filePath];
				}
				else if (!match) {
				...
				if (sourceMap.sourcesContent) {
					sourceMap.sourcesContent.forEach(function (source, idx) {
						normalizedPath = path.normalize(sourceMap.sources[idx]);
						sourcePath = normalizedPath.startsWith(sourceMapDir)
							? path.relative(process.cwd(), normalizedPath)
							: path.join(sourceMapDir, sourceMap.sources[idx]);
						getSourceCoverage(srcCoverage, sourcePath).data.code = source;
						if (sourceStore) {
							sourceStore.set(sourcePath, source);
						}
					});
				}
